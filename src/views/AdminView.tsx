...
type TodayOrderLocationSummaryRow = {
  ...
  return (
    <section className="panel admin-module-panel">
      ...
      <div className="summary-board">
        ...
        <div className="summary-card">
          <h3>后台说明</h3>
          <p>可编辑内容会先保留在当前页面草稿中，点击顶部“保存修改”后才会正式写入数据库。</p>
          <p>订单状态修改、查看支付截图、删除订单和重置临时密码属于即时操作，无需等待统一保存。</p>
        </div>
      </div>
    </section>
  )
...
function buildTodayOrderLocationSummary(
  orders: OrderRecord[],
  categories: string[],
): TodayOrderLocationSummaryRow[] {
  return DELIVERY_LOCATIONS.map((location) => {
    const counts = categories.reduce<Record<string, number>>((current, category) => {
      current[category] = 0
      return current
    }, {})
    ...
    orders
      .filter((order) => order.deliveryLocation === location)
      .forEach((order) => {
        order.items.forEach((item) => {
          const category = item.mealCategory?.trim() || '未分类'
...
export function AdminView(props: AdminViewProps) {
  ...
  const moduleCards: ModuleCard[] = [
    ...
    { id: 'today-orders', title: '今日订单', description: '查看今天的下单明细与配送统计。', meta: `${props.orders.length} 笔订单` },
    { id: 'config', title: '系统配置', description: '维护截单时间、汇率、支付说明和收款码。', meta: `截单 ${props.orderDeadlineHour}:00` },
    { id: 'orders', title: '订单管理', description: '处理订单状态并查看订单截图。', meta: `${props.orders.length} 笔订单` },
    { id: 'users', title: '用户管理', description: '维护用户资料并重置临时密码。', meta: `${props.managedUsers.length} 位用户` },
    { id: 'menu', title: '今日菜单', description: '调整菜品内容、价格和上架状态。', meta: `${props.menu.filter((meal) => meal.availableToday).length} 个上架` },
    { id: 'payments', title: '付款记录', description: '查看付款登记与支付截图。', meta: `${props.payments.length} 条记录` },
    { id: 'stats', title: '每日统计', description: '查看统计快照与补充收支备注。', meta: `${props.dailyStats.length} 天数据` },
  ]
  ...
  const filteredOrders = useMemo(() => {
    ...
    return props.orders.filter((order) =>
      [
        ...
        getMealSummaryText(order),
        order.paymentStatus,
      ].some((value) => value.toLowerCase().includes(keyword)),
    )
  }, [props.adminSearch, props.orders])

  const todayOrderDigest = useMemo(
    () =>
  ...
  const todayOrderCategories = useMemo(() => {
    ...
    props.orders.forEach((order) => {
      order.items.forEach((item) => {
        categories.add(item.mealCategory?.trim() || '未分类')
      })
    })
    return Array.from(categories)
  }, [props.orders])

  const todayOrderLocationSummary = useMemo(
    () => buildTodayOrderLocationSummary(props.orders, todayOrderCategories),
  ...
  const statsSummary = useMemo(() => {
    ...
    return rows.reduce(
      ...
      { rows, totalSold: 0, totalCost: 0, totalProfit: 0, paidOrders: 0, extraIncome: 0, extraExpense: 0 },
    )
      ...
      <section className="panel admin-module-panel">
        ...
        <div className="table-wrap responsive-card-wrap">
          <table className="responsive-table">
            ...
            <tbody>
              {todayOrderLocationSummary.some((row) => row.total > 0) ? (
                todayOrderLocationSummary.map((row) => (
                  <tr key={row.location}>
                    <td data-label="配送地点">{row.location}</td>
                    {todayOrderCategories.map((category) => (
                      <td data-label={category} key={`${row.location}-${category}`}>
                        {row.counts[category] ?? 0}
                      </td>
                    ))}
                    <td data-label="合计">{row.total}</td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td className="table-empty" colSpan={todayOrderCategories.length + 2}>
                    今日暂无订单统计
                  </td>
                </tr>
              )}
  ...
  return (
    <main className="admin-layout single-column admin-console">
      <section className="panel admin-toolbar">
        <div className="admin-toolbar-head">
          <div className="admin-toolbar-title">
            ...
            <h1>管理后台</h1>
            <p className="admin-toolbar-subtext">在原有后台基础上统一处理订单、菜单、支付记录、用户资料及每日经营数据。</p>
          </div>
        ...
        <div className="admin-toolbar-summary">
          ...
          <article className={`admin-toolbar-stat ${props.hasPendingChanges ? 'pending' : ''}`}>
            ...
            <p>{props.hasPendingChanges ? '当前页面有未保存内容。' : '当前内容已与数据库同步。'}</p>
          </article>
      ...
      {props.hasPendingChanges ? (
        <div aria-live="polite" className="admin-save-float" role="status">
          ...
          <div className="confirm-card menu-editor-dialog">
            <div className="panel-head">
              <div>
                ...
                <p className="panel-subtext">修改完成后，点击顶部“保存修改”即可同步到数据库。</p>
              </div>
            </div>
